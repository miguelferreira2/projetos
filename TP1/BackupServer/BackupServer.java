...
public class BackupServer {
    ...
    public static void main(String args[]) {
        ...
        if (check) { // ja existe um servidor a correr, este servidor vai iniciar como backup
            ...
            new Thread() {
                public void run() {
                    try (DatagramSocket aSocket = new DatagramSocket(5600)) {
                        while (true) {
                            byte[] buffer = new byte[1000];
                            ...
                            InetAddress aHost = InetAddress.getByName("localhost");
                            DatagramPacket reply = new DatagramPacket(byteReply, byteReply.length, aHost, 5500);
                            aSocket.send(reply);
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (splitted[0].equals(user)) {
                    if (splitted[1].equals("--"))
                        ...
                        return (splitted[1].replace("/BackupServer", currentpath));
                }
            }
        } catch (IOException ex) {
            System.out.format("I/O exception: ", ex);
    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
                ...
                updateLastDir(login[0], currentpath.replace(System.getProperty("user.dir"), "/BackupServer"));
                ...
                if (data.equals("sls")) {
                    ...
                    if (data.equals("scd")) {
                        out.writeUTF(currentpath.replace(System.getProperty("user.dir"), "/BackupServer"));
                    }
                        ...
                        if (!cd[0].equals("scd"))
                            ...
                            if (cd[1].equals(".")) {
                                ...
                                updateLastDir(login[0],
                                        currentpath.replace(System.getProperty("user.dir"), "/BackupServer"));
